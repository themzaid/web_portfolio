        ...
        <div className="container mx-auto max-w-[1200px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr] gap-8 md:gap-12 items-start pt-4">
            <div>
              ...
              <h1 className="text-[40px] md:text-[72px] font-serif font-normal leading-[1.05] tracking-[-0.02em] text-foreground mb-6">
                {project.title}
              </h1>
          ...
          >
            ...
            <div className="absolute inset-0 flex items-center justify-center font-serif text-[64px] md:text-[140px] text-white/5 tracking-[-0.05em] select-none">
              {project.bgLabel}
            </div>
            ...
            <div className="absolute inset-x-6 md:inset-x-8 top-8 bottom-0 bg-[#111215] rounded-t-[18px] border-[1px] border-white/10 border-b-0 overflow-hidden shadow-[0_24px_64px_rgba(0,0,0,0.4)]">
              ...
              <div className="px-6 py-7 md:p-8 flex flex-col items-start h-[calc(100%-40px)] relative z-10">
                ...
                <h2 className="font-serif text-[24px] md:text-[36px] leading-none text-white max-w-[10ch] mb-3 md:mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[220px_1fr] lg:grid-cols-[240px_1fr] gap-8 md:gap-10 lg:gap-10">
            ...
            <aside className="md:sticky md:top-[88px] self-start space-y-6 w-full">
                ...
                {caseStudy.blocks.map((block, i) => (
                  <li key={i}>
                    <button
                      ...
                      className={`w-full flex items-center text-left justify-between px-3.5 py-2.5 rounded-[10px] text-[13px] transition-colors ${activeSection === i ? 'bg-secondary text-foreground font-medium' : 'text-muted-foreground hover:bg-secondary/40 hover:text-foreground'}`}
                    >
              ...
              <div className="p-5 border-[1px] border-border rounded-[10px] bg-card">
              ...
              {project.links && project.links.length > 0 && (
                <div className="p-5 border-[1px] border-border rounded-[10px] bg-card flex flex-col gap-3">
                  <div className="text-[11px] tracking-[0.14em] uppercase text-muted-foreground mb-1">Links</div>

                  {project.links.map((link, idx) => {
                    let Icon = ExternalLink;
                    let styleClass = "bg-transparent border-[1px] border-border text-foreground hover:bg-secondary";
                    ...
                    if (idx === 0) {
                      styleClass = "bg-foreground text-background hover:bg-primary border-[1px] border-transparent";
                    }
                    ...
                    return (
                      <a
                        ...
                        className={`flex items-center justify-between w-full px-4 py-3 rounded-lg text-[11px] font-semibold tracking-[0.08em] uppercase transition-colors ${styleClass}`}
                      >
                        {link.label}
            ...
            <div className="grid gap-6">
              {caseStudy.blocks.map((block, i) => (
                ...
                >
                  <div className="text-[11px] tracking-[0.16em] mb-4 text-muted-foreground/60">
                    0{i + 1} / {block.navLabel || block.title.split(' ')[0]}
                  </div>
                  <h3 className="font-serif text-[22px] md:text-[32px] font-normal tracking-[-0.02em] leading-[1.2] mb-4 text-foreground">
                    {block.title}
                  </h3>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-6">
                      {block.outcomes.map((outcome, index) => (
                        <div key={index} className="p-4 md:p-5 border-[1px] border-border rounded-[10px] bg-white/[0.02]">
                          <strong className="block font-serif text-[28px] md:text-[32px] font-normal text-primary mb-1 leading-none">{outcome.value}</strong>
